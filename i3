...
bindsym $ms+r exec i3-input -F 'rename workspace to "%s"' -P 'New name: '
...
bar {
        status_command python3 ~/.i3pystatus.py
        workspace_buttons yes
}

#bindsym $mod+m exec thunderbird-es
bindsym $mod+b exec firefox
for_window [class="Firefox" title="Library"] floating enable
for_window [class="Firefox" title="Cookies Manager*"] floating enable
...
bindsym $ms+e exec setxkbmap es
